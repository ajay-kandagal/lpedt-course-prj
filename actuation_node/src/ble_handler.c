  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_gatt_server_attribute_value_id:
      ...
      if (gattdb_relay_control == evt->data.evt_gatt_server_characteristic_status.characteristic) {
          ...
          switch(data_recv)
          {
            case BT_CODE_TURN_ON:
              app_log_info("Relay turned ON\n");
              ble_data.value = BT_CODE_TURN_ON;
              sl_bt_external_signal(EVT_RELAY_ON);
              break;
            case BT_CODE_TURN_OFF:
              app_log_info("Relay turned OFF\n");
              ble_data.value = BT_CODE_TURN_OFF;
              sl_bt_external_signal(EVT_RELAY_OFF);
              break;
            default:
